(function () {
  ...
  Asteroid.prototype.registerCollision = function (bullet, response) {
    ...
    for (i = 0; i < this.newVertices.length && this.newVertexIdxs.length < 2; i++) {
      ...
      p1 = this.newVertices[i];
      p2 = this.newVertices[j];
      ...
      if (this.newVertices[j].x < xIntercept && xIntercept < this.newVertices[i].x) {

        this.newVertexIdxs.push(j);
        this.newVertices.splice(j, 0, new SAT.V(
          xIntercept, bulletSlope*xIntercept + bulletIntercept));
      } else if (this.newVertices[j].x < xIntercept && xIntercept < this.newVertices[i].x) {

        this.newVertexIdxs.push(j);
        this.newVertices.splice(j, 0, new SAT.V(
          xIntercept, bulletSlope*xIntercept + bulletIntercept));
      } else if (this.newVertices[i].x < xIntercept && xIntercept < this.newVertices[j].x) {
      ...
      if (this.newVertices.length - this.numSides === 2) {
        break
      }
    }
