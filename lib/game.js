    ...
    this.bullets.forEach(function (bullet) {
      this.asteroids.forEach(function (asteroid) {
        ...
        if (asteroid.isCollidedWith(bullet, response)) {
          if (bullet.collidedAsteroid && bullet.collidedAsteroid === asteroid) {
            bullet.vertex2 = bullet.pos.clone();

            edge = response.edge;
            asteroid.polygon.edges.indexOf(edge[0]);
          } else {
            asteroid.remove = true;
            bullet._remove = true;
            bullet.collidedAsteroid = asteroid;
            bullet.vertex1 = bullet.pos.clone();
          }
        }
      });
    }, this);
