    ...
    this.bullets.forEach(function (bullet) {
      my.asteroids.forEach(function (asteroid) {
        ...
        if (asteroid.isCollidedWith(bullet, response)) {
          ...
          // if it's not yet collided with a bullet
          if (!asteroid._collidedBullet) {
            asteroid.registerCollision(bullet, response);
            asteroid.edge1 = response.edge;

          // if it has collided, and the bullet is the same, and the bullet is exiting
          } else if (asteroid._collidedBullet
              && asteroid.edge1.x != response.edge.x
              && asteroid.edge1.y != response.edge.y
              && asteroid._collidedBullet === bullet
              && response.bInA == false) {
